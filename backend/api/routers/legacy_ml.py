"""Unprefixed ML routes for backward compatibility with older clients / NestJS examples."""
...
router = APIRouter(tags=["legacy-ml"])


@router.post("/classify_role", response_model=ClassifyRoleResponse)
def classify_role_legacy(req: ClassifyRoleRequest):
...
def match_score_legacy(req: MatchScoreRequest):
...
def rank_legacy(req: RankCandidatesRequest, db: Session = Depends(get_db)):
